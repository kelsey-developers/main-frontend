...
import { getAgentRegistrationConfig, getMyAgentRegistration, type MyAgentRegistration } from '@/services/agentRegistrationService';
import { CheckCircleIcon, ArrowRightIcon, ArrowUpTrayIcon, DocumentTextIcon, UserGroupIcon, UserIcon, CurrencyDollarIcon, ChartBarIcon } from '@heroicons/react/24/outline';
...
function BecomeAnAgentContent() {
  ...
  const { user, userProfile, isAgent, isAdmin, isFinance, isInventory, isHousekeeping, isOperations, isFrontdesk, roleLoading } = useAuth();

  const recruitedById = searchParams.get('recruitedBy') ?? '';

  const hasNonGuestRole = isAgent || isAdmin || isFinance || isInventory || isHousekeeping || isOperations || isFrontdesk;

  // Only available to guests; redirect if user has any non-guest role
  useEffect(() => {
  ...
  const [justSubmitted, setJustSubmitted] = useState(() =>
    typeof window !== 'undefined' && !!sessionStorage.getItem('agentRegistrationJustSubmitted')
  );
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && sessionStorage.getItem('agentRegistrationJustSubmitted')) {
      sessionStorage.removeItem('agentRegistrationJustSubmitted');
    }
  }, []);
  ...
  useEffect(() => {
    (async () => {
      try {
        const [config, myReg] = await Promise.all([
          getAgentRegistrationConfig(),
          user ? getMyAgentRegistration().catch((): MyAgentRegistration => ({ hasRegistration: false })) : Promise.resolve({ hasRegistration: false } as MyAgentRegistration),
        ]);
        setRegistrationFee(config.registrationFee);
        if (myReg.hasRegistration) {
          setStep('success');
          setExistingRegistration({
            email: myReg.email,
            fullname: myReg.fullname,
            status: myReg.status,
          });
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (recruitedById) setForm((f) => ({ ...f, recruitedBy: recruitedById }));
  }, [recruitedById]);
  ...
  const validate = (): boolean => {
    ...
    if (!form.feeProofFile) err.feeProofFile = 'Proof of payment is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fd.append('agreeTerms', String(form.agreeTerms));
      if (form.recruitedBy?.trim()) {
        fd.append('recruitedBy', form.recruitedBy.trim());
      }
      ...
      if (user) {
        if (typeof sessionStorage !== 'undefined') {
          sessionStorage.setItem('agentRegistrationJustSubmitted', '1');
        }
        setStep('success');
        setExistingRegistration({
          email: user.email,
          fullname: userProfile?.fullname || [user.firstName, user.lastName].filter(Boolean).join(' '),
          status: 'pending',
        });
        router.refresh();
      } else {
        ...
        const loginResult = await loginAction(form.email.trim(), form.password, '/become-an-agent');
  ...
  if (step === 'success') {
    const isJustSubmitted = justSubmitted;
    return (
      <div className="flex items-center justify-center min-h-[60vh] px-4 py-8">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-6 max-w-md w-full text-center">
          <div className="w-14 h-14 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-green-100">
            <SolidCheckCircleIcon className="w-7 h-7 text-green-500" />
          </div>
          <h2 className="text-xl font-bold text-gray-900 mb-2">
            {isJustSubmitted ? 'Application Received!' : "You've Already Applied"}
          </h2>
          <p className="text-sm text-gray-600 mb-5">
            ...
            {existingRegistration?.status !== 'approved' && existingRegistration?.status !== 'rejected' &&
          ...
          <div className="bg-gray-50 rounded-xl p-4 text-left space-y-2.5 mb-5 border border-gray-100">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{existingRegistration?.email ?? form.email}</p>
              </div>
            </div>
            {(existingRegistration?.fullname || form.fullname) && (
              <div className="flex items-center gap-2">
                <UserIcon className="w-4 h-4 text-gray-400 shrink-0" />
                <div>
                  <p className="text-[10px] text-gray-400 uppercase">Name</p>
                  <p className="text-sm font-medium text-gray-900">{existingRegistration?.fullname ?? form.fullname}</p>
                </div>
              </div>
            )}
            {form.recruitedBy && (
              <div className="flex items-center gap-2">
                <UserGroupIcon className="w-4 h-4 text-gray-400 shrink-0" />
                <div>
                  <p className="text-[10px] text-gray-400 uppercase">Referred By</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[10px] text-gray-400 uppercase">Fee</p>
          ...
          <button
            ...
            className="w-full py-3 text-sm font-semibold text-white bg-[#0B5858] hover:bg-[#094a4a] rounded-xl transition-colors flex items-center justify-center gap-2 cursor-pointer"
          >
            Back to Homepage
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-xl p-6 sm:p-10 relative">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-500 text-sm mt-1">
                {user ? 'Upload your payment proof and optionally add your referrer.' : 'Please fill in your details accurately to speed up the approval process.'}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              {!user && (
                <>
                  {/* Section: Personal Info - only for guests */}
                  ...
                  {/* Section: Account Setup - only for guests */}
              ...
              <div className="space-y-5" id="field-feeProofFile">
                <div className="flex items-center gap-2 mb-4">
                  <div className="w-8 h-8 rounded-full bg-[#0B5858]/10 text-[#0B5858] flex items-center justify-center font-bold text-sm">{user ? '1' : '3'}</div>
              ...
              {/* Section: Referred By - shown for both */}
              <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100">
                ...
                <input
                  ...
                  placeholder="Agent User ID (from invite link)"
                />
                {recruitedById && (
              ...
              <div className="space-y-6 pt-2" id="field-agreeTerms">
                ...
                {errors.submit && (
                  <p className={`${errorClass} flex items-center gap-1`}>
                    <span className="w-1 h-1 rounded-full bg-red-500" /> {errors.submit}
                  </p>
                )}
                <button
                  type="submit"
