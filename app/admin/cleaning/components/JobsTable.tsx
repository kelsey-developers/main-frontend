  ...
  if (jobs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-gray-100">
        ...
        <p className="text-xs text-gray-400">Try adjusting your filters or schedule a new job.</p>
      </div>
    );
  }
        ...
        <thead>
          <tr className="bg-gray-50 border-b border-gray-100">
            {['Property / Unit', 'Type', 'Scheduled', 'Cleaner', 'Duration', 'Status', 'Actions'].map((h) => (
              <th key={h} className="px-4 py-3 text-left text-[10px] font-bold text-gray-400 uppercase tracking-wide whitespace-nowrap first:pl-5 last:pr-5">
                {h}
              </th>
            ))}
          </tr>
            ...
            return (
              <tr key={job.id} className={`hover:bg-gray-50/50 transition-colors ${job.status === 'cancelled' ? 'opacity-50' : ''}`}>
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${tc.bgColor} ${tc.color} whitespace-nowrap`}>
                    {tc.label}
                  </span>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap ${sc.classes}`}>
                ...
                <td className="px-4 py-3 pr-5">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <Link
                      ...
                      className="px-2.5 py-1 rounded-lg text-[11px] font-semibold text-gray-600 bg-gray-50 hover:bg-gray-100 border border-gray-200 transition-colors cursor-pointer whitespace-nowrap"
                    >
                      View
                    </Link>
                    {canAssign && (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg text-[11px] font-semibold text-[#0B5858] bg-[#0B5858]/5 hover:bg-[#0B5858]/10 border border-[#0B5858]/10 transition-colors cursor-pointer whitespace-nowrap"
                      >
                        Assign
                      </button>
                    )}
                    {canVerify && (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg text-[11px] font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 transition-colors cursor-pointer whitespace-nowrap"
                      >
                        Verify
                      </button>
                    )}
                    {canCancel && (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg text-[11px] font-semibold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 transition-colors cursor-pointer whitespace-nowrap"
                      >
                        Cancel
                      </button>
                    )}
