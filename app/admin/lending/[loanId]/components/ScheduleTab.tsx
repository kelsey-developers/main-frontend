...
> = {
  upcoming: { label: 'Upcoming',  classes: 'bg-blue-50 text-blue-700 border border-blue-200',    dot: 'bg-blue-400' },
...
export default function ScheduleTab({ loan, schedules, onPaymentRecorded }: Props) {
  ...
  return (
    <div className="space-y-4">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              {['#', 'Due Date', 'Principal', 'Interest', 'Total Due', 'Balance', 'Status', 'Paid On', 'Actions'].map((h) => (
                <th key={h} className="px-3 py-2.5 text-left text-[10px] font-bold text-gray-400 uppercase tracking-wide whitespace-nowrap first:pl-4 last:pr-4">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-gray-50">
            {schedules.map((s) => {
              ...
              return (
                <tr key={s.period} className={`hover:bg-gray-50/50 transition-colors ${s.status === 'overdue' ? 'bg-red-50/30' : ''}`}>
                  ...
                  <td className="px-3 py-3 font-mono text-gray-700">{fmt(s.principalDue)}</td>
                  <td className="px-3 py-3 font-mono text-gray-700">{fmt(s.interestDue)}</td>
                  <td className="px-3 py-3 font-mono font-bold text-gray-900">{fmt(s.totalDue)}</td>
                  <td className="px-3 py-3 font-mono text-gray-600">{fmt(s.remainingBalance)}</td>
                  ...
                  <td className="px-3 py-3 pr-4">
                    {canPay && (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg text-[11px] font-bold text-[#0B5858] bg-[#0B5858]/5 hover:bg-[#0B5858]/10 border border-[#0B5858]/10 transition-colors cursor-pointer whitespace-nowrap"
                      >
                        Record Payment
                      </button>
                    )}
      ...
      {/* Record Payment Modal triggered from a row */}
      {paymentTarget && (
        <RecordPaymentModal
          loan={loan}
