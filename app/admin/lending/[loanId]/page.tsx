  ...
  if (!loan) {
    return (
      <div className="py-12 text-center">
        ...
        <Link href="/admin/lending" className="mt-3 inline-block text-sm font-semibold text-[#0B5858] hover:underline">← Back to Lending</Link>
      </div>
    );
  }
    ...
    <div className="py-8 space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-5">
          ...
          <div className="flex items-start justify-between gap-3 mb-5">
            <div>
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${roleBadge.classes}`}>
                  {roleBadge.label}
                </span>
              </div>
              <p className="text-xs font-mono text-gray-400 mb-1">{loan.id}</p>
              <p className="text-sm text-gray-500 max-w-sm">{loan.purpose}</p>
            </div>
            <div className="flex flex-col items-end gap-2 shrink-0">
              ...
              {loan.collateral && (
                <p className="text-xs text-gray-400 italic text-right">Collateral: {loan.collateral}</p>
              )}
            </div>
          </div>
          ...
          {/* Disbursement info */}
          {loan.disbursedAt && (
            <div className="mt-4 grid grid-cols-3 gap-3 pt-4 border-t border-gray-100 text-xs">
              <div>
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wide mb-0.5">Disbursed Amount</p>
                <p className="font-bold text-gray-900">{fmt(loan.disbursedAmount)}</p>
              </div>
              <div>
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wide mb-0.5">Processing Fee</p>
                <p className="font-bold text-gray-900">{fmt(loan.processingFee)}</p>
              </div>
              <div>
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wide mb-0.5">Disbursed On</p>
                <p className="font-bold text-gray-900">{fmtShort(loan.disbursedAt)}</p>
              </div>
            </div>
          )}
          ...
          {loan.approvedAt && (
            <div className="mt-3 flex items-center gap-1.5 text-xs text-gray-400">
              <svg className="w-3.5 h-3.5 text-[#0B5858]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-wrap gap-2">
          {loan.status === 'pending' && (
            <>
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-bold text-white bg-[#0B5858] hover:bg-[#0d7a7a] transition-colors cursor-pointer disabled:opacity-50 shadow-sm shadow-[#0B5858]/20"
              >
              ...
              <button
                ...
                disabled={actionLoading}
                className="flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-bold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 transition-colors cursor-pointer disabled:opacity-50"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {loan.status === 'active' && (
            <button
              ...
              disabled={actionLoading}
              className="px-4 py-2 rounded-full text-sm font-bold text-[#0B5858] bg-[#FACC15]/10 hover:bg-[#FACC15]/20 border border-[#FACC15]/30 transition-colors cursor-pointer disabled:opacity-50"
            >
              Mark as Overdue
            </button>
          )}
          {(loan.status === 'active' || loan.status === 'overdue') && (
            <button
              ...
              disabled={actionLoading}
              className="px-4 py-2 rounded-full text-sm font-bold text-gray-500 bg-gray-50 hover:bg-gray-100 border border-gray-200 transition-colors cursor-pointer disabled:opacity-50"
            >
              Write Off
            </button>
          )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="relative px-5 border-b border-gray-100">
          <div className="flex">
            {([
            ...
            ] as { key: Tab; label: string; count: number }[]).map(({ key, label, count }, i) => (
              <button
                key={key}
              ...
              >
                {label}
                <span className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold ${activeTab === key ? 'bg-[#0B5858] text-white' : 'bg-gray-100 text-gray-400'}`}>
                  {count}
                </span>
              </button>
            ))}
          ...
          {/* Sliding indicator */}
          <div
            className="absolute bottom-0 h-[2px] bg-[#0B5858] rounded-full transition-all duration-200"
